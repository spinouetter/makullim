...
function cell(slot, i, cx, top, d, big){
  ...
  const pw = big ? Math.max(30, d*0.5) : Math.max(22, d*0.6), ph = big ? 18 : 15, py = nameY + (big ? 7 : 6);
...
// 빌리(김우진)는 배경 포스터가 곧 본인 → 이름·관극수 텍스트 없이 배경으로만.
const CX0 = 300, CW = 528;   // 콘텐츠 폭 x 300..828
P(`<text x="565" y="455" text-anchor="middle" font-family="IBM Plex Sans KR Medm, sans-serif" font-size="14" letter-spacing="1" fill="${BLUE_D}">김우진 빌리와 함께한 무대</text>`);
...
heading("SEAT MAP", 566, bandTop + 14, 258, 12.5);
P(`<rect id="fn-seatbox" x="566" y="${(bandTop + 24).toFixed(1)}" width="258" height="262" fill="none"/>`);
